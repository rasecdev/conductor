...
| `artefatos` | Linhas de `CLAUDE.md`/regras de convenção, `PROGRESSO.md` e `PLANO.md` que citam ferramenta de artefato (Notion, Miro, Figma, ClickUp, Excalidraw, draw.io, pen.dev), tipo de artefato (board, diagrama, fluxo de tela, plano de teste) ou arquivo de diagrama — registro ou recusa; arquivos `*.mmd`, `*.drawio`, `*.excalidraw`, `*.puml` e documentos de QA (`docs/qa/*.md`, `qa/*.md`, `*test-plan*`, `*plano-de-teste*`, `*casos-de-teste*`) no repositório. |
